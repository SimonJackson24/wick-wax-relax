...
import {
  ...
  ChevronRight as TreeViewIcon,
} from '@mui/icons-material';
import axios from 'axios';
import HierarchicalCategories from './HierarchicalCategories';

const HierarchicalSearchFilters = ({
  filters = {},
  onFiltersChange,
  onApplyFilters,
  onClearFilters,
  ...
  const [selectedFilters, setSelectedFilters] = useState({
    ...
    categories: filters.categories || [], // Support multiple categories
  ...
  const [expandedCategories, setExpandedCategories] = useState(['seasons', 'collections', 'aroma-profiles']);
  ...
  const loadAvailableFilters = async () => {
    try {
      ...
      const categoriesResponse = await axios.get('/api/categories/hierarchical/filters');
      setAvailableFilters(prev => ({
        ...prev,
        hierarchicalCategories: categoriesResponse.data.filters
      }));
  ...
  const getActiveFilterCount = () => {
    ...
    if (Object.keys(selectedFilters.attributes).length > 0) {
      count += Object.values(selectedFilters.attributes).flat().length;
    }
  ...
  const renderCategoryFilter = () => {
    if (!availableFilters.hierarchicalCategories) {
      return null;
    }

    const { level1, level2 } = availableFilters.hierarchicalCategories;

    return (
      <Accordion defaultExpanded>
        <AccordionSummary expandIcon={<ExpandMoreIcon />}>
          <Typography variant="subtitle1">Categories</Typography>
          {(selectedFilters.category || selectedFilters.categories.length > 0) && (
            <Chip
              label={`${selectedFilters.categories.length || 1} selected`}
              size="small"
              onDelete={() => {
        ...
        <AccordionDetails>
          <TreeView
            ...
            defaultExpandIcon={<TreeViewIcon />}
            expanded={expandedCategories}
            onNodeToggle={(event, nodeIds) => setExpandedCategories(nodeIds)}
                      ...
                      label={
                        <Box sx={{ display: 'flex', alignItems: 'center' }}>
                          <FormControlLabel
                            control={
                              <Checkbox
                                checked={selectedFilters.categories.includes(child.id)}
...
export default HierarchicalSearchFilters;