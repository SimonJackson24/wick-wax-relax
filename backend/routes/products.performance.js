...
// Get all products with variants - WITH CACHE
...
router.get('/:id', [
  param('id').isUUID()
], cacheMiddleware(3600, cacheKeys.productDetail), async (req, res) => {
  try {
...
router.get('/category/:categorySlug', cacheMiddleware(1800, (req) => `products:category:${req.params.categorySlug}`), async (req, res) => {
  try {
...
router.get('/categories/all', cacheMiddleware(3600, cacheKeys.categories), async (req, res) => {
  try {
...
router.post('/', async (req, res) => {
  try {
    ...
    await invalidateCache.product('new-product-id');
